...
`pando remove [--force] [branch ...]` removes registered topic worktrees but never deletes their local branch refs. No arguments removes the current topic; explicit branches select registered topics. Dirty worktrees require `--force`, and removing the current worktree emits the primary path after deletion. Before deleting anything it measures each target in one timed step, then names the worktree and its size while Git removes it and reports the total reclaimed in the outro. Measurement counts allocated blocks, counts a hard-linked file once, never follows symlinks, and excludes nested registered worktrees; `~` marks a total that skipped an unreadable subtree. Structured JSON removal measures nothing and its contract is unchanged.

`pando clean [--dry-run]` is `remove`'s interactive front end: a multi-select picker over every registered worktree except the primary, with a `SIZE` column showing the disk each one occupies. Space toggles a worktree, `Ctrl-A` toggles everything matching the current filter, `Ctrl-S` cycles the sort (adding size, largest-first, to the shared modes), and `Enter` removes the checked set — with nothing checked it exits having removed nothing. Sizes are measured in the background and fill in as they land; they count allocated blocks, count a file reached through a second hard link once, never follow symlinks, and exclude nested registered worktrees, so a value is the space the removal actually returns. `~` marks a total that skipped an unreadable subtree. Locked, prunable, detached, missing, and inaccessible worktrees are shown with their state but cannot be checked. A checked worktree with uncommitted changes is named in the confirmation, which defaults to no, before its changes are discarded. Removal itself is unchanged: `pre-remove` hooks run, branches are retained, each worktree and its size are named while Git removes it, the space reclaimed is reported in the outro, and removing the current worktree emits the primary path. The picker's measurements are handed to the removal, so `clean` never walks the same trees twice. `clean` supports human output only; use `remove` with explicit branches for scripted removal. A shell whose integration predates `clean` needs one `pando install` re-run before the wrapper will `cd` out of a worktree `clean` removed.
