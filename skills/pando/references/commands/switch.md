...
The global `--verbose` flag disables animated progress and writes nested phase and Git subprocess timings to stderr. It can appear before or after the subcommand, for example `pd switch main --verbose`.

The binary writes **only** the successful destination path to stdout, with a
trailing newline. Prompts, warnings, hook output, and explicitly enabled verbose diagnostics go to stderr. This is
