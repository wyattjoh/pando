...
When the success is the last thing the rail says, use `ui::run_timed_completing` with `ui::Completion::Outro` instead. The wrapper then retires the spinner silently and renders the completed message as the closing outro, styled like an info line (accent bar, plain text) with the same muted elapsed suffix. Worktree creation picks `Outro` when no post-create commands are pending and `Step` when setup output follows, so the sequence still closes exactly once. Never follow an `Outro` run with another outro.
