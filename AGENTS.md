...
| `clean.rs` | Interactive multi-select worktree cleanup: the picker loop, its background measurement workers, and the confirmation that hands selected branches and their sizes to `lifecycle` |
| `size.rs` | Bounded worktree disk measurement shared by `clean` and `remove`, plus the formatting of one size and of a set's total |
...
| `include.rs` | `.worktreeinclude` copying of ignored files from the invoking worktree into a newly created one |
...
**`clean` is an interactive adapter over one removal.** `clean.rs` owns the multi-select picker, its background measurement workers, and the confirmation; `lifecycle::remove_reported` still owns planning, hook approval, mutation, branch retention, the destination write, the per-removal timed step, the reclaimed total, and the itemized per-target report a fail-fast batch produces, so `remove` and `clean` cannot drift. The picker reads keys on demand through a request channel instead of parking a thread inside a terminal read, which is what leaves stdin free for the confirmation prompt that follows — do not replace it with a free-running reader. Size ordering is recomputed only on the sort key, never when a measurement lands, so rows cannot shuffle under the cursor mid-selection. Size is deliberately not a `SortMode`: it is measured per run and must never become a configurable `default-sort`. `clean` has no JSON leaf; `--output json` is refused and scripted removal goes through `remove`.

**Sizes are presentation-only, and measured once.** `size.rs` owns the one bounded walk: allocated blocks rather than apparent lengths, a hard-linked file counted once, symlinks never followed, nested registered worktrees excluded, unreadable subtrees skipped and reported as a partial total. A size never authorizes, blocks, orders, or classifies a removal — only how one is described. Human `remove` measures its targets in a single timed step before mutation, then names each target's size while deleting it and the batch total in the outro; `clean` passes the sizes its picker already measured through `RemovalReport` so the same trees are never walked twice. JSON removal measures nothing and its contract is unchanged. Every background worker is cancelled and joined before mutation begins, so no walk is ever live while Git deletes a tree.
